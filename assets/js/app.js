...
/* harmony import */ var _chenfengyuan_datepicker__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! @chenfengyuan/datepicker */ "./node_modules/@chenfengyuan/datepicker/dist/datepicker.js");
...
/* harmony import */ var _lib_slick_min_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./lib/slick.min.js */ "./src/assets/js/lib/slick.min.js");
/* harmony import */ var _lib_slick_min_js__WEBPACK_IMPORTED_MODULE_4___default = /*#__PURE__*/__webpack_require__.n(_lib_slick_min_js__WEBPACK_IMPORTED_MODULE_4__);
...
var baHost = document.getElementById("baHost"); // new
//If we come from index page make search

fetch(endpoint).then(function (blob) {
  return blob.json();
}).then(function (data) {
  cities.push.apply(cities, _toConsumableArray(data)); //get parameter query when we go from index to search page
...
function displayMatches(event) {
  ...
  var matchArray = findMatches(searchInput.value, cities);
  clearMarkers(); //check if there is city we search for
  ...
  if (matchArray.length == 0) {
    ...
    var html = matchArray.map(function (place) {
      ...
      var cardHtml = cardTmpl.replace(/🦄name🦄/ig, place.personInfo.name).replace(/🦄city🦄/ig, place.city).replace(/🦄state🦄/ig, place.state).replace(/🦄friends🦄/ig, place.personInfo.friends).replace(/🦄languages🦄/ig, place.personInfo.languages).replace(/🦄references🦄/ig, place.personInfo.references).replace(/🦄img🦄/ig, place.personInfo.img).replace(/🦄room🦄/ig, place.personInfo.accomodaion);
      return cardHtml;
    }).join('');
    hostResults.innerHTML = html; // show destination under the map

    destination.innerHTML = "".concat(matchArray[0].city, ", ").concat(matchArray[0].state); // show number of hosts in location

    baHost.innerHTML = "".concat(matchArray.length, " hosts in ").concat(matchArray[0].city, ", ").concat(matchArray[0].state);
    map.markers = []; // console.log(markersCoords);
...
module.exports = __webpack_require__(/*! c:\Front-end\42-git-team\job-abroad\src\assets\js\app.js */"./src/assets/js/app.js");
